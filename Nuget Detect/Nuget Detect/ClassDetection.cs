...
namespace ONNX_detect1
{
    public class DataItem
    {
        public DataItem(string filename, string label, float x, float y, float w, float h, float conf)
        {
    ...
    public class ClassDetection
    {
        ...
        static async Task Process(CancellationTokenSource cts)
        {
        ...
        public static async Task<(List<Image<Rgb24>>, List<DataItem>)> ObjectDetection(List<Image<Rgb24>> images, string[] filenames)
        {
            ...
            var processTask = Task.Run(async () => { return Process(cts); });
        ...
        static async Task DownLoadNN(CancellationTokenSource cts)
        {
        ...
        static (Image<Rgb24>, DataItem) Prediction((Image<Rgb24>, string) image_filename, InferenceSession session)
        {
            ...
            return (resized, new DataItem($"detected_{filename}", labels[max_ind], x, y, w, h, max_conf));
        }
